...
def normalize_white_balance(im):
    ...
    """
    _EPS = 1e-8
    channel_mean = torch.mean(im, dim=(-3, -2), keepdim=True)
    max_of_mean, _ = torch.max(channel_mean, dim=-1, keepdim=True)
...
def quantize_8(image):
    ...
    q8 = to_pil_image(image)
    q8 = to_tensor(q8).to(torch.uint8)
    return q8.float() / 255.0