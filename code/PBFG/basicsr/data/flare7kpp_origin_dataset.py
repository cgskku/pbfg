...
from basicsr.data.data_util import normalize_white_balance
...
class Flare_Image_Loader(data.Dataset):
	def __init__(self, image_path ,transform_base,transform_flare,mask_type=None):
		...
		self.transform_flare=transforms.Compose([transforms.RandomAffine(degrees=(0,360),scale=(transform_flare['scale_min'],transform_flare['scale_max']),translate=(transform_flare['translate']/1024,transform_flare['translate']/1024),shear=(-transform_flare['shear'],transform_flare['shear'])),
	...
	def __getitem__(self, index):
		...
		base_img = gain * base_img + flare_DC_offset
...
class Flare7kpp_origin_Pair_Loader(Flare_Image_Loader):
...
@DATASET_REGISTRY.register()
class Image_Pair_Loader_origin(data.Dataset):
    def __init__(self, opt):
        super(Image_Pair_Loader_origin, self).__init__()
        self.opt = opt
...
@DATASET_REGISTRY.register()
class ImageMask_Pair_Loader03(Image_Pair_Loader_origin):
    def __init__(self, opt):
        Image_Pair_Loader_origin.__init__(self,opt)
        self.opt = opt
